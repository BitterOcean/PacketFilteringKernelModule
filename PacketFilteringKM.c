                         ...
                         int(*okfn)(struct sk_buff *))
{
    ...
    if(ip_header->protocol==17)
    {
        ...
        printk(KERN_INFO "Packet recieved from %s", tmp);
        if(numberReads)
        {
            for (; i < numberReads ; i++)
            {
            ...
            if(configType)
            {
                if (flag)
                {
                    printk(KERN_INFO"(%04d-%02d-%02d %02d:%02d:%02d) Got a new Packet and accepted it due to being in whitelist\n"
                          , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
                    return NF_ACCEPT;
                }
                else
                {
                    printk(KERN_INFO"(%04d-%02d-%02d %02d:%02d:%02d) Got a new Packet and dropped it due to not being in whitelist\n"
                          , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
                    return NF_DROP;
                }
            }
            else
            {
                if(flag)
                {
                    printk(KERN_INFO"(%04d-%02d-%02d %02d:%02d:%02d) Got a new Packet and dropped it duo to being in blacklist\n"
                          , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
                    return NF_DROP;
                }
                else
                {
                    printk(KERN_INFO"(%04d-%02d-%02d %02d:%02d:%02d) Got a new Packet and accepted it duo to not being in blacklist\n"
                          , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
                    return NF_ACCEPT;
                }
            }
        ...
        {
            printk(KERN_INFO"(%04d-%02d-%02d %02d:%02d:%02d) Got a new Packet and accepted it due to no list!\n"
                  , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
            return NF_ACCEPT;
        }
    }
    else if(ip_header->protocol == 6)
    {
        ...
        sprintf(tmp,"%s:%d\n",source_string,source_port);
        printk(KERN_INFO "(%04d-%02d-%02d %02d:%02d:%02d) Packet recieved from %s"
              , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec , tmp);
        if(numberReads)
        {
            for (; i < numberReads ; i++)
            {
    ...
    if(!sock_buff)
    {
        printk(KERN_INFO"(%04d-%02d-%02d %02d:%02d:%02d) Got a new Packet and dropped it due to that if\n"
              , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
        return NF_DROP;
    }

    printk(KERN_INFO"(%04d-%02d-%02d %02d:%02d:%02d) Got a new Packet and accepted it due to that idk\n"
          , tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
    return NF_ACCEPT;
}
